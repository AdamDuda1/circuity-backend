...
router.get('/get', (req, res) => {
    var _a, _b;
    try {
        const limitValue = Number((_a = req.query.limit) !== null && _a !== void 0 ? _a : 18); // no. of elements on the page
        ...
        const stmt = db.prepare('SELECT id, content, author, name, description, visibility, created_at FROM projects ORDER BY created_at DESC LIMIT ? OFFSET ?');
 ...
 */
router.post('/single-project', (req, res) => {
    try {
        ...
        const project = stmt.get(secret.trim());
        if (!project) {
 ...
 */
router.get('/admin/list', admin_auth_1.auth, (req, res) => {
    try {
        const stmt = db.prepare('SELECT id, content, author, name, description, visibility, created_at FROM projects ORDER BY created_at DESC');
 ...
 */
router.patch('/admin/:id', admin_auth_1.auth, (req, res) => {
    try {
 ...
 */
router.delete('/admin/:id', admin_auth_1.auth, (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id) || id < 1) {
            return res.status(400).json({ error: 'Invalid project id' });
